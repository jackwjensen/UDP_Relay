...
- **Target frameworks are mixed on purpose** (Core `netstandard2.0`; Console/harnesses/tests multi-target `net8.0;net9.0;net10.0`; Service `net4.7.2`). .NET 10 is the primary LTS target; 8 and 9 are kept for compatibility. Keep the `<TargetFrameworks>` list in sync across the four SDK-style multi-targeted projects.
